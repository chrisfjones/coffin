...
(function() {
  var CoffeeScript, buildCfnPath, checkChar, coffinChar, colors, commander, compileCommand, compileTemplate, convertCfnTemplate, convertCommand, createStack, crossChar, exec, fs, generateCoffinOutputFileName, generateOutputFileName, generateTempFileName, parseTemplate, path, pretty, printCommand, showHelp, spawn, stackCommand, updateOrCreateStack, validateArgs, validateCommand, validateTemplate, writeJsonTemplate, _ref,
    _this = this,
    __slice = [].slice;
  ...
  convertCfnTemplate = function(source, callback) {
  ...
  generateCoffinOutputFileName = function(source) {
    var base, filename;
    base = commander.output || path.dirname(source);
  ...
  convertCommand.action(function(cfnTemplate) {
    validateArgs();
    return convertCfnTemplate(cfnTemplate, function(converted) {
      var fileName;
      process.stdout.write("" + coffinChar + " " + cfnTemplate + " -> ");
      fileName = generateCoffinOutputFileName(cfnTemplate);
      return writeJsonTemplate(converted, fileName, function() {
        return process.stdout.write("" + fileName);
      });
    });
  });
